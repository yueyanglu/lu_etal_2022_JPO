...
c     
     ...
     $     timetot,time1,time2
      ...
      integer,parameter :: whatTracer = 2 ! 1: orig 10 c; 2: test c
c
      real amin,amax
      integer iord, margin
C       
...
!$OMP END PARALLEL DO
      ...
      ! read full forcing
      call getdat_archo(flnm, forcflg) 
      call flush(lp)
!$OMP PARALLEL DO PRIVATE(i,j,k)
      do k = 1, kk
         do j = 1-nbdy, jj+nbdy
...
c           READ mean & full forc (u/vflx, dpm, dpio). flnm will display
            write(lp,*) '---' 
            !-------------------  read mean forcing
            call getdat_archo(flnm, mforcflg) 
            call flush(lp)
            !  isopycnal mass flux [m3], times by delt1 
            q0 = delt1
            ...
            do k = 1, kk
               do j = 1-margin, jj+margin
                  do i = 1-margin, ii+margin
                     if(ip(i,j).eq.1) then
                        hordiv(i,j,k) = uflxm(i+1,j,k) - uflxm(i,j,k)
     &                       + vflxm(i,j+1,k) - vflxm(i,j,k)
                        hordiv(i,j,k) = hordiv(i,j,k) * scp2i(i,j)
                        ! horiz div of eddy vol flx
                        hordivE(i,j,k) = uflx(i+1,j,k) - uflxm(i+1,j,k) 
     &                                - (uflx(i,j,k) - uflxm(i,j,k)) 
     &                                + (vflx(i,j+1,k) - vflxm(i,j+1,k))
     &                                - (vflx(i,j,k) - vflxm(i,j,k))
                        hordivE(i,j,k) = hordivE(i,j,k) * scp2i(i,j)
                     endif
                  enddo
               enddo
            enddo
            ...
            do j = 1-margin, jj+margin
               do i = 1-margin, ii+margin
                  if(ip(i,j).ne.0) then
                     ...
                     do k = 1, kk
                        ka = max(1,k-1)
                        vertdiv = - hordiv(i,j,k) + ( dpiim(i,j,k) 
     $                       - dpiom(i,j,k) ) / float(nintrp)
...
!$OMP PARALLEL DO PRIVATE(i,j,k)
            do j = 1-nbdy, jj+nbdy
               do i = 1-nbdy, ii+nbdy
                  if (ip(i,j).ne.0) then
                     do k = 1, kk
                        dp(i,j,k,1)=dpiim(i,j,k)*(1.-q1)+dpiom(i,j,k)*q1
                        dp(i,j,k,2)=dpiim(i,j,k)*(1.-q2)+dpiom(i,j,k)*q2
                     enddo
                  endif
               enddo
            enddo
...
c --- HORIZONTAL DIFFUSION
     ...
     &                  tracer(1-nbdy,1-nbdy,k,ktr), k, 0 )
c --- lateral open boundary conditions if needed
               ...
               write(lp,*) 'cmass all z:', cm1, cm2, (cm2-cm1)/cm1
