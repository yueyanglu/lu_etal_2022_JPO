...
***********************************************************************
      ...
      subroutine tridrhs(h,delt1,yo,diff,ghat,ghatflux,tri,nlayer,rhs)
c      use mod_xc         ! HYCOM communication interface
c      use mod_cb_arrays  ! HYCOM saved arrays
c      use mod_tracer
      implicit none
c
...
c --- solve tridiagonal matrix.
      ...
      if(bet.eq.0.) then
         print*, '** algorithm for solving tridiagonal matrix fails'
         print*, '** i,j=',i,j !global grid point
         print*, '** bet=',bet
         print*, '** k=',k,' tcc=',tcc(k),' tcu=',tcu(k),
     &        ' gam=',gam(k)
c        call flush(lp)
c        call xchalt('(tridmat)')
c               stop '(tridmat)'
*       bet=1.E-12
      endif
